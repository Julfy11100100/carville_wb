...
- ✅ Скрипт синхронизации категорий в БД app.scripts.get_category_tree
...
Чтобы не заморачиваться с настройкой системы для локального запуска, можно запускать через docker:

Собрать билд, если ранее не был собран
```
docker compose build
```
Запустить скрипт
```
docker compose run -d --rm wb_manager python -m app.scripts.get_category_tree
```
